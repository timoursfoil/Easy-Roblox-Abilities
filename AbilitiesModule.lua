...
local abilities = { -- Table of the abilities
	Kick = { -- Basic ability as an example
		...
		Hitbox = Vector3.new(5.479, 10.182, 3.647), -- Hitbox of the ability (the range that it will hit enemies from)
...
function abilities.KickAbility(player: Player) -- Function to Kick
	...
	for i, thing in pairs(hitbox:GetTouchingParts()) do
		if thing.Parent:FindFirstChild("Humanoid") and not touched[thing.Parent] then
